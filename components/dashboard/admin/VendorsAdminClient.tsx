...
export default function VendorsAdminClient({ initialVendors }: { initialVendors: VendorRow[] }) {
    ...
    const createVendor = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setForm({ name: "", email: "", password: "", phone: "", companyName: "", city: "", isActive: true })
            router.refresh()
        } catch (err: any) {
            ...
            <form onSubmit={createVendor} className="bg-white border border-gray-200 rounded-xl p-4 grid gap-3 md:grid-cols-2">
                ...
                <div className="relative flex items-center">
                    <input
                        ...
                        placeholder="Password (optional, blank to auto-invite)"
                        type={showPassword ? "text" : "password"}
                        minLength={6}
                        className="w-full rounded-md border border-gray-300 pl-3 pr-10 py-2 text-sm"
                    />
                    <button
                        type="button"
            ...
            <div className="md:hidden space-y-3">
                {initialVendors.map((vendor) => (
                    <div key={vendor.id} className="bg-white border border-gray-200 rounded-xl p-4 space-y-2">
                        ...
                        <div className="text-xs text-gray-600">City: {vendor.vendorProfile?.city || "-"}</div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {initialVendors.map((vendor) => (
                            <tr key={vendor.id}>
                                ...
                                <td className="px-4 py-3 text-sm text-gray-700">{vendor.vendorProfile?.city || "-"}</td>
                                <td className="px-4 py-3">
                        ...
                        {initialVendors.length === 0 && (
                            <tr>
                                <td className="px-4 py-8 text-center text-sm text-gray-500" colSpan={5}>No vendors yet.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
