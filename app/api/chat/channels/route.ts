...
export async function GET() {
    try {
        ...
        return NextResponse.json({ error: "Database connection temporarily unavailable, retrying..." }, { status: 503 })
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        return NextResponse.json({ error: "Database connection temporarily unavailable" }, { status: 503 })
    }
}
...
async function ensureDefaultChannel(userId: number) {
    let defaultChannel = await withDbRetry(() =>
        db.chatChannel.findFirst({
    ...
    if (!defaultChannel) {
        ...
        defaultChannel = await withDbRetry(() =>
            db.chatChannel.create({
                data: {
                    name: "General",
